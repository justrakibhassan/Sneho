...
export const updateProfile = async (req, res) => {
    try {
        ...
        // 1. Common Data
        const { name, phone, profilePicture, 
        // Parent Specific
        location, minBudget, maxBudget, situation, 
        // Babysitter Specific
        bio, experienceYears, hourlyRate, } = req.body;
        let updateData = {
            name,
            phoneNumber: phone,
            profilePicture,
            ...
            updateData.babysitter = {
                upsert: {
                    ...
                    update: {
                        ...
                        skills: req.body.skills,
                        preferredAgeGroup: req.body.preferredAgeGroup,
                        handlesSpecialNeeds: req.body.handlesSpecialNeeds !== undefined
                            ? Boolean(req.body.handlesSpecialNeeds)
                            : undefined,
                        energyCompatibility: req.body.energyCompatibility
                            ? parseInt(req.body.energyCompatibility)
    ...
    try {
        ...
        if (role === "PARENT") {
            ...
            if (parent) {
                ...
                const totalSpent = bookings.reduce((sum, b) => sum + (parseFloat(b.totalAmount.toString()) || 0), 0);
            ...
            if (sitter) {
                ...
                stats.balance = completedBookings.reduce((sum, b) => sum + (parseFloat(b.totalAmount.toString()) || 0), 0);
            }
        }
        res.status(200).json({
            success: true,
        ...
        if (role === "PARENT") {
            ...
            if (parent) {
                ...
                activities = bookings.map((booking) => ({
                    id: booking.id,
                    type: "Booking",
                    description: `Booking with ${booking.babysitter.user.name} for ${new Date(booking.startTime).toLocaleDateString()}`,
            ...
            if (sitter) {
                ...
                activities = bookings.map((booking) => ({
                    id: booking.id,
                    type: "Booking",
                    description: `Booking request from ${booking.parent.user.name} for ${new Date(booking.startTime).toLocaleDateString()}`,
                    timestamp: booking.createdAt,
                    status: booking.status,
                }));
                // Get recent sessions
