...
export const sendBookingRequestEmail = async (email, name) => {
    const mailOptions = {
    ...
    try {
        ...
        console.error(`[Email Error] Failed to send booking request email to ${email}:`, err);
    }
};
export const sendBookingStatusEmail = async (email, status) => {
    const mailOptions = {
        from: `"Sneho Support" <${process.env.SMTP_USER}>`,
        to: email,
    ...
    try {
        ...
        console.error(`[Email Error] Failed to send booking status email to ${email}:`, err);
    }
};
export const sendMeetingLinkEmail = async (email, name, link, bookingId) => {
    const mailOptions = {
        from: `"Sneho Support" <${process.env.SMTP_USER}>`,
        to: email,
    ...
    try {
        ...
        console.error(`[Email Error] Failed to send meeting link email to ${email}:`, err);
    }
};
export const sendResetPasswordEmail = async (email, token) => {
    ...
    try {
        ...
        if (process.env.NODE_ENV !== "production") {
            ...
            fs.appendFileSync("reset_links.log", `Email: ${email}, Link: ${resetUrl} (Real email sent: ${info.messageId})\n`);
        }
    }
    catch (err) {
        ...
        if (process.env.NODE_ENV !== "production") {
            try {
                const fs = await import("fs");
                fs.appendFileSync("reset_links.log", `Email: ${email}, Link: ${resetUrl} (Real email FAILED: ${err.message})\n`);
            }
            catch (logErr) { }
        }
        throw err;
