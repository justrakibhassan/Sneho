...
export const initSessionSocket = (socketIO: Server) => {
  io = socketIO;
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("session:join", async (data: { sessionId: string | number; userId: string | number; role: string }) => {
    ...
    socket.on("session:status_change", async (data: { sessionId: string | number; status: string; userId: string | number; metadata?: any }) => {
        ...
        if (status === "paused") {
          ...
          if (session && session.status === "PAUSED") {
            ...
            const pausedTime = Math.floor(
              (now.getTime() - new Date(session.pauseTime!).getTime()) / 1000
            );
    ...
    socket.on("location:update", async (data: { sessionId: string | number; latitude: string | number; longitude: string | number; accuracy?: string | number; timestamp?: string | number }) => {
    ...
    socket.on("location:batch_update", async (data: { sessionId: string | number; locations: any[] }) => {
    ...
    socket.on("session:get_duration", async (data: { sessionId: string | number }) => {
    ...
    socket.on("session:request_state", async (data: { sessionId: string | number }) => {
      const { sessionId } = data;
