...
export const initSocket = (httpServer: HttpServer) => {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("join_support_room", (userId: string | number) => {
      socket.join(`support_${userId}`);
    });

    // Explicit request for agent
    socket.on("request_agent", (data: { userId: string | number; userName?: string }) => {
    ...
    socket.on(
      "send_message",
      ({ userId, userName, message, isAgentActive }: { userId: string | number; userName?: string; message: string; isAgentActive: boolean }) => {
    ...
    socket.on("admin_reply", ({ userId, message }: { userId: string | number; message: string }) => {
      io.to(`support_${userId}`).emit("receive_message", {
        sender: "agent",
        message,
        timestamp: new Date(),
