 ...
 * @returns Ratio 0-1
 */
function checkAvailabilityOverlap(requiredDays: string | string[], sitterAvailability: any[]): number {
  if (!requiredDays || (Array.isArray(requiredDays) && requiredDays.length === 0)) return 0;
  if (!sitterAvailability || sitterAvailability.length === 0) return 0;
  ...
  const matches = normalizedDays.filter((day) => 
    sitterDays.some(sd => sd.includes(day) || day.includes(sd))
  );
 ...
 * @returns Score 0-1
 */
function matchPersonality(child: any, sitter: any): number {
  let score = 0;
 ...
 */
export async function findMatchingSitters(parentId: number) {
  try {
    ...
    const scoredSitters = sitters.map((sitter) => {
      ...
      if (
        ...
        const distance = calculateDistance(
          Number(parent.latitude),
          Number(parent.longitude),
      ...
      if (parent.requiredDays) {
        const availabilityRatio = checkAvailabilityOverlap(
          parent.requiredDays,
          sitter.availability
        );
