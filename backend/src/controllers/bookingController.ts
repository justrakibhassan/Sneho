...
export const updateBookingStatus = async (req: Request, res: Response) => {
  try {
    ...
    const { sendBookingStatusEmail } = await import(
...
export const logGPS = async (req: Request, res: Response) => {
  try {
    const bookingId = parseInt(req.params.id as string);
...
export const updateMeetingLink = async (req: Request, res: Response) => {
  try {
    ...
    if (updated.babysitter?.user?.email) {
      sendMeetingLinkEmail(
        updated.babysitter.user.email,
        updated.babysitter.user.name || "Babysitter",
        meetingLink,
        bookingId
      );
    }
...
export const getSitterEarnings = async (req: Request, res: Response) => {
  try {
    const userRequest = req.user as IUser;
    const userId = userRequest.id;
    ...
    const totalEarnings = bookings.reduce((sum, booking: any) => {
      const amount =
    ...
    const transactions = bookings.map((b: any) => ({
      id: b.id,
      amount:
        (Number(b.payment?.amount) || Number(b.totalAmount)) +
