...
export const getConversations = async (req: Request, res: Response) => {
  try {
    ...
    const conversations = bookings.map((b: any) => ({
      bookingId: b.id,
  ...
  try {
    ...
    const message = await prisma.message.create({
      data: {
        ...
        isRead: false,
        receiverId:
          senderId === booking.parent.userId
    ...
    res.status(201).json({ success: true, message: message as any });
  } catch (error) {
...
export const getStreamChatToken = async (req: Request, res: Response) => {
  try {
    const userRequest = req.user as IUser;
...
export const createOrGetConversation = async (req: Request, res: Response) => {
  try {
    ...
    const channelId = `conv-${conversation.id}-${new Date(
